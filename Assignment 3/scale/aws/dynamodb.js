...
const tableName2 = "n12150801-assigment-reformatdata"
...
async function addReformatMetadata(idToken, videoID, metadata){
    const compositeSortKey = `${idToken}#${videoID}`;
...
async function queryReformatUser(idToken) {
    const command = new DynamoDBLib.QueryCommand({
        TableName: tableName2,
        KeyConditionExpression: "#username = :username and begins_with(#compositeKey, :idToken)",
        ExpressionAttributeNames: {
...
async function getReformatVideo(idtoken_videoid) {
    const command = new DynamoDBLib.GetCommand({
        TableName: tableName2,
        Key: {
        "qut-username": qutUsername,
...
async function deleteReformatVideo(idToken, videoID) {
    const compositeSortKey = `${idToken}#${videoID}`;
    const command = new DynamoDBLib.DeleteCommand({
        TableName: tableName2,
        Key: {
